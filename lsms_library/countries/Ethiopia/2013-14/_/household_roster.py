...
def make_i(x):
    """Create household id from household_id2"""
    try:
        if pd.isna(x):
            return ''
        return str(x).strip()
...
# Load data - use household_id2 as per household_characteristics.py
idxvars = dict(i=('household_id2', make_i))

myvars = dict(sex=('hh_s1q03', sex_mapper),
              age='hh_s1q04_a',
              relation=('hh_s1q02', relation_mapper_ethiopia))

df = df_data_grabber('../Data/sect1_hh_w2.dta', idxvars, convert_categoricals=False, **myvars)

# Create pid from individual_id2
from lsms_library.local_tools import get_dataframe
df_temp = get_dataframe('../Data/sect1_hh_w2.dta', convert_categoricals=False)
df_temp = df_temp.set_index('household_id2')

# Reset index to add individual_id2
df = df.reset_index()
df = df.merge(df_temp[['individual_id2']].reset_index(), left_on='i', right_on='household_id2', how='left')
df['pid'] = df['individual_id2'].astype(str)

# Drop helper columns
df = df.drop(columns=['household_id2', 'individual_id2'], errors='ignore')
...
df.index = pd.MultiIndex.from_tuples(
    [('2013-14',) + idx for idx in df.index],
