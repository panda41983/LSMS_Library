...
def make_i(x):
    """Create household id from hhid (already exists as single value)"""
    # x is a single value (hhid), not a list
    try:
        if isinstance(x, (list, tuple, pd.Series)):
            val = x[0] if not pd.isna(x[0]) else 0
        else:
            val = x if not pd.isna(x) else 0
        return str(int(val))
...
def make_age(x):
    ...
    if not pd.isna(x[0]):
        return x[0]
    # Calculate from birth year (2022 for wave 2, 2021 for wave 1)
    if not pd.isna(x[1]):
        return 2022 - x[1]  # Using 2022 as base year for 2021-22 survey
    return np.nan

def relation_mapper_2021(x):
    """Map relation codes to English for 2021-22 wave"""
    # Mapping for 2021-22: codes 1-11
    mapping = {
        1: 'Head',
        2: 'Spouse',
        3: 'Child',
        ...
        6: 'Grandparent',
        7: 'Sibling',
        8: 'Other relative',  # Nephew/Niece mapped to Other relative
        9: 'Other relative',
        10: 'Non-relative',
        11: 'Non-relative',
...
# Load all data first
idxvars = dict(i=('hhid', make_i),
               vague='vague')

myvars = dict(sex=('s01q01', sex_mapper),
              age=(['s01q04a', 's01q03c'], make_age),
              relation=('s01q02', relation_mapper_2021))

df_all = df_data_grabber('../Data/s01_me_bfa2021.dta', idxvars, convert_categoricals=False, **myvars)

# Check if we have data
if len(df_all) == 0:
    raise ValueError("No data loaded from s01_me_bfa2021.dta! Check file and column mappings.")

# Reset index to work with columns
df_all = df_all.reset_index()

# Wave 1 (vague = 1 or 1.0): 2021
...
w1.index = pd.MultiIndex.from_tuples(
    [('2021-22',) + idx for idx in w1.index],
    names=['t'] + w1.index.names
)

# Wave 2 (vague = 2 or 2.0): 2022
...
w2.index = pd.MultiIndex.from_tuples(
    [('2021-22',) + idx for idx in w2.index],
