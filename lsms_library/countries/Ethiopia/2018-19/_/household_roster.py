...
def sex_mapper(x):
    """Convert sex: 1=male, 2=female (lowercase m/f) - based on household_characteristics.py converter"""
    try:
        if pd.isna(x):
            return ''
        # Handle numeric values (1/2 or 1.0/2.0)
        # Based on household_characteristics.py: lambda x: ['f','m'][x[0]=='1']
        # This means 1 = 'm' (male), 2 = 'f' (female) - opposite of usual!
        try:
...
# Load data - based on household_characteristics.py, s1q01 is relation, s1q02 is sex
idxvars = dict(i=('household_id', make_i))

myvars = dict(sex=('s1q02', sex_mapper),
              age='s1q03a',
              relation=('s1q01', relation_mapper_ethiopia))

df = df_data_grabber('../Data/sect1_hh_w4.dta', idxvars, convert_categoricals=False, **myvars)

# Create pid from individual_id
from lsms_library.local_tools import get_dataframe
df_temp = get_dataframe('../Data/sect1_hh_w4.dta', convert_categoricals=False)
...
df.index = pd.MultiIndex.from_tuples(
    [('2018-19',) + idx for idx in df.index],
